...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $sql = "SELECT id, title, type, listingType, price, status, state, city, street, description, contactName, phone, email, galleryPhotos, publish FROM building_material WHERE id = $id LIMIT 1";
...
<section class="w3-content w3-white w3-padding w3-card" style="max-width: 800px; margin:auto; margin-top: 20px;">
  <h2 class="mb-4"><?php echo $id ? "Edit" : "Add"; ?> Building Material Listing</h2>
  <form id="propertyForm" method="POST" enctype="multipart/form-data" action="save_building_material.php">
